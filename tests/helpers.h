...
public:
  RandomVectorGenerator(int size, double _min, double _max) : min{_min}, max{_max}
  {
    ...
    current_value = current_value * (max - min) / 2 + Eigen::VectorXd::Constant(size, (min + max) / 2);
  ...
  bool next() override
  {
    ...
    current_value = current_value * (max - min) / 2 + Eigen::VectorXd::Constant(current_value.size(), (min + max) / 2);
    return true;
  }
...
public:
  RandomMatrixGenerator(int nRows, int nCols, double _min, double _max) : min{_min}, max{_max}
  {
    ...
    current_value = current_value * (max - min) / 2 + Eigen::MatrixXd::Constant(nRows, nCols, (min + max) / 2);
    static_cast<void>(next());
  }
